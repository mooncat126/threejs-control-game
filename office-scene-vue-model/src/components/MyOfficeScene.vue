...
export default {
  ...
  setup() {
    ...
    const sphereMaterial = new THREE.MeshStandardMaterial({ color: 0xff5a00 });
    ...
    // 调整每个圆球的大小和位置
    sphere1.scale.set(0.4, 0.4, 0.4);
    sphere1.position.set(-1, 3.4, -2.9); // 调整到杯子上方

    sphere2.scale.set(0.8, 0.8, 0.8);
    sphere2.position.set(-0.7, 3.5, -2.9); // 调整到杯子上方

    sphere3.scale.set(0.6, 0.6, 0.6);
    sphere3.position.set(-0.7, 3.3, -3); // 调整到杯子上方
    ...
    const cupAnimate = () => {
      const animateSphere = (sphere, delay) => {
      ...
      // 开始跳动动画
      animateSphere(sphere1, 0);
      animateSphere(sphere2, 0.1);
      animateSphere(sphere3, 0.2);
    };

    // 添加 XYZ 辅助线
    // const axesHelper = new THREE.AxesHelper(5); // 5 是辅助线的长度，可以根据需要调整
